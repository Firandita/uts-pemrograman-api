...
<body>
	...
	<script>
		...
		const currentUser = <?php echo json_encode($sessionUsername); ?>;
		const localKeyEndpoints = 'custom_endpoints_v1_' + currentUser;
		const localKeyCopies = 'total_copy_count_v1_' + currentUser;
		const localKeyApiKey = 'api_key_' + currentUser;
		...
		document.getElementById('sendBtn').addEventListener('click', async () => {
        	...
        	const bodyObj = JSON.parse(bodyText);
       	 	bodyUsername = (bodyObj.username || '').trim().toLowerCase();
    		} catch (e) {}

    		const isOwnLogin = bodyUsername === currentUser.trim().toLowerCase();

    		if (isOwnLogin) {
        	tryApiKeyEl.value = parsed.api_key;
        	localStorage.setItem(localKeyApiKey, parsed.api_key);
   	 		} else {
       
        	console.warn('Login berhasil tapi untuk user lain, API key tidak diupdate.');
    		}
}
				} catch (err) {
		...
		tryApiKeyEl.value = localStorage.getItem(localKeyApiKey) || sessionApiKey || '';
